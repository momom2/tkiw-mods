...
impl Registry {
    ...
    /// Every object in the registry, from a snapshot that is refreshed at most
    /// every [`RESCAN_GAP`].
    ///
    /// The rate limit is the point. [`Registry::objects`] is O(mask), and the mask is
    /// bounded only by a sanity cap -- so a walk can be a million slot reads. A
    /// *miss* cannot be cached, because an object that does not exist yet may exist
    /// a second later, so without a rate limit every miss re-walks. A caller polling
    /// four object names during startup, when the registry is open but still sparse,
    /// was measured at **three seconds per poll** -- enough to stop the game, and it
    /// did.
    fn snapshot(&self) -> Vec<(i32, usize)> {
        let Ok(mut snap) = walk_cache().lock() else { return self.objects() };
        if let Some((at, ref objects)) = *snap {
            if at.elapsed() < RESCAN_GAP {
        ...
        // Report the shape of the registry once. The mask is what decides the cost of
        // every walk, and it is not knowable from the executable.
        static SAID: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
        if !SAID.swap(true, std::sync::atomic::Ordering::Relaxed) {
            crate::findln!(
                "object registry: mask {}, {} objects, walked in {:.2}ms",
                self.mask,
                objects.len(),
                took.as_secs_f64() * 1000.0
            );
        }
        *snap = Some((std::time::Instant::now(), objects.clone()));
        objects
    }
...
/// How often the whole registry may be walked. See [`Registry::snapshot`].
///
/// Two seconds is chosen against what the snapshot is *for*: watching objects appear
/// and disappear as the game changes phase, which happens on the scale of seconds.
/// Anything needing to know sooner should hold the pointer it was given rather than
/// asking again.
const RESCAN_GAP: std::time::Duration = std::time::Duration::from_secs(2);

type WalkCache = std::sync::Mutex<Option<(std::time::Instant, Vec<(i32, usize)>)>>;

fn walk_cache() -> &'static WalkCache {
    static CACHE: std::sync::OnceLock<WalkCache> = std::sync::OnceLock::new();
