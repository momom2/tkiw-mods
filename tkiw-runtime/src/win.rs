...
extern "system" {
    ...
    pub fn CreateMutexW(attrs: *mut c_void, owner: i32, name: *const u16) -> Handle;
    pub fn OpenMutexW(access: u32, inherit: i32, name: *const u16) -> Handle;
    ...
    fn FlushInstructionCache(process: Handle, addr: *const c_void, size: usize) -> i32;
    fn GetCurrentProcess() -> Handle;
}

/// Tell the CPU that code changed underneath it.
///
/// x86 is coherent about this in practice, so it is easy to leave out and get away
/// with. It is still the documented contract, it costs nothing at patch time, and
/// leaving it out is the kind of omission that works on every machine you own.
...
/// Letter keys, for hotkey chords. A mod's chord should carry two modifiers so
/// it cannot collide with the game's own bindings, which use letters, digits,
/// space and the arrows unmodified.
pub const fn vk_letter(c: u8) -> i32 {
