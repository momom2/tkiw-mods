...
impl Feature for FortificationsCap {
    ...
    fn summary(&self) -> &'static str {
        "Prevent Brick Factories from increasing castle max hp indefinitely."
    }
...
mod tests {
    ...
    fn the_allowance_is_one_of_two_named_values() {
        ...
        let cfg = momomod_kit::config::Config::parse("[feature.fortifications_cap]\ncap = total\n");
        f.configure(&cfg.section("fortifications_cap")).expect("total is accepted");
        assert!(f.allowance == Allowance::Total);

        let cfg =
            momomod_kit::config::Config::parse("[feature.fortifications_cap]\ncap = per_factory\n");
