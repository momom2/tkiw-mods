...
/// Where the registry reads each feature's on/off and settings.
///
/// The manager's `ConfigSet` (many mods, the mirror) implements it, and so does
/// a single mod's config wrapper -- so [`Registry::apply`] is written once and
/// works for both. `module` is the mod a feature belongs to; a single-mod
/// plugin ignores it (all its features are one module).
pub trait FeatureCfg {
...
impl Registry {
    ...
    ///
    /// Generic over the config source ([`FeatureCfg`]) so the same runner serves
    /// the manager -- whose `ConfigSet` spans many mods -- and a single mod
    /// plugin, which has one `.ini` and one module.
    pub fn apply<C: FeatureCfg>(&mut self, rt: &Runtime, cfg: &C) {
    ...
    pub fn tick(&mut self, rt: &Runtime, now: Instant) {
        ...
        for slot in &mut self.slots {
            ...
            if took > HARD_LIMIT || slot.cost > BUDGET {
                // A self-paced feature (the picker) is never killed for cost: it
                // governs its own, and being switched off mid-run left it having
                // acted once and then gone dormant. It is still held to the
                // pathological limit above, and its cost is still noted once.
                if slot.feature.self_paced() {
                    if slot.strikes == 0 {
                        logln!(
                            "[{name}] slow: {:.1}ms this call, averaging {:.2}ms - it paces \
                             itself, so it is left to run rather than switched off",
                            took.as_secs_f64() * 1000.0,
                            slot.cost.as_secs_f64() * 1000.0
                        );
                    }
                    slot.strikes = 1; // "in a slow spell", but never counts toward death
                } else {
                    slot.strikes += 1;
