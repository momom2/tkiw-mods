//! Save snapshots.
//!
//! This mod's whole job is to act on the player's behalf inside a run, so the
//! thing genuinely at risk is not any game file -- it is the save. Every launch
//! takes a snapshot **before** the mod does anything, into the mod's own
//! folder, so a session that goes wrong can be rolled back to the state it
//! started from.
//!
