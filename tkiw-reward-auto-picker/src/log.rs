//! The log.
//!
//! The mod resolves rewards invisibly, so the log is the only record of what it
//! did on the player's behalf. That makes it load-bearing rather than a
//! debugging aid, and it is not optional.
...
const FILE: &str = "picker.log";
const ROTATE_AT: u64 = 4 * 1024 * 1024;

static LOCK: Mutex<()> = Mutex::new(());

pub fn write(line: &str) {
    let Some(path) = home::file(FILE) else {
...
pub fn write_unlocked(line: &str) {
    let Some(path) = home::file(FILE) else { return };
    if let Ok(mut fh) = OpenOptions::new().create(true).append(true).open(&path) {
