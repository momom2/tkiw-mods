...
fn identify() {
    identity::set(Identity {
        ...
        orphan_note: "tkiw_momomod_error.log",
    });
}
...
fn reload_config(rt: &Runtime, force: bool) {
    ...
    let files: Vec<std::path::PathBuf> = std::iter::once(dir.join(config::KIT_FILE))
        .chain(features::MODS.iter().map(|m| dir.join(config::mod_file(m.name))))
        .collect();
    ...
    for m in features::MODS {
        ...
        if set.kit_config().section_named("mods").get(m.name).is_none() {
            logln!(
                "config: {} does not list mod {:?}; loaded at its default ({}). Add it \
        ...
        if !set.mod_enabled(m.name, m.default_on) {
            logln!("config: mod {:?} is switched off - {} feature(s) not loaded.", m.name, mine.len());
            continue;
        }
