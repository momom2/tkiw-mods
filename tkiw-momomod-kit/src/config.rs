...
//! config/qol.ini          the QoL mod's features
...
//! sections, same keys. So `config/qol.ini` under the kit and `qol.ini` beside a
//! standalone QoL mod are the same document, and a player moving between the two
//! keeps their settings. Nothing kit-specific may leak into a mod's file; that is
//! what `config/momomod.ini` is for.
//!
...
impl ConfigSet {
    ...
    /// Whether a mod is loaded at all, from `[mods]` in the kit's file.
    ///
    ...
    pub fn mod_enabled(&self, module: &str, default: bool) -> bool {
        ...
        match section.bool(&module.to_ascii_lowercase(), default) {
            Ok(v) => v,
            Err(why) => {
                logln!("config: [mods] {why} - using the default ({default})");
                default
            }
        }
    }
