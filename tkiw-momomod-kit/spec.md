...
and the scheduler all work; three features exist (`fast_boot`, `timeline`, `profiler`);
the auto-picker shares the runtime layer. 75 tests across the workspace.
...
2. **`fast_boot`** — the startup win the profiler identified. ~30% off the time to the
   main menu, measured both ways. See [`analysis/FINDINGS.md`](analysis/FINDINGS.md).
