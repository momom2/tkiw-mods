...
Build the zip that ships.
    ...
    python package.py           build dist/momomod-kit-<version>.zip
...
DLL_CANDIDATES = [
    os.path.join(ROOT, "target", "release", "momomod_kit.dll"),
    os.path.join(KIT_DIR, "target", "release", "momomod_kit.dll"),
]
...
def main():
    if "--list" in sys.argv:
        print("ships:")
        for src, name in [("momomod_kit.dll (built)", "momomod_kit.dll")] + SHIPS:
            print(f"  {name:28} <- {src}")
        print("\nstays behind:")
    ...
    out = os.path.join(DIST, f"momomod-kit-{version()}.zip")
    with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as z:
        z.write(dll, "momomod_kit.dll")
