...
BUILT_DLL_CANDIDATES = [
    os.path.join(MOD_DIR, "..", "target", "release", "momomod_kit.dll"),
    os.path.join(MOD_DIR, "target", "release", "momomod_kit.dll"),
    os.path.join(MOD_DIR, "dist", "momomod_kit.dll"),
]
...
def main():
    ...
    print()
    print("On first launch the kit writes one config file per mod into:")
    print(f"    {os.path.join(MOD_DIR, 'config')}")
    print("momomod.ini is the kit itself; the rest are the mods, and each is the")
    print("same file that mod would read installed on its own. A pre-existing")
    print("momomod.ini in the folder above is migrated into them, not ignored.")
    print("Then run  python configure.py  for a window to set everything from,")
    print("or edit those files in any text editor.")
    print("Anything that changes the rules starts switched off, so the kit changes")
    print("nothing about the game. What it did is in momomod.log next to it.")
    print()
